...
def random_add_dense_to_config(config, dense_limit, hard_limit, hidden_layer_limit, inits, dense_activation_functions,
    ...
    for i in range(dense_limit):
        ...
        if np.random.uniform(0,1) < 0.1:
            ...
            if reg == 'l1l2':
                config["dense_weight_regularizers"].append((reg, np.random.uniform(0,1) * 0.1, np.random.uniform(0,1) * 0.1))
            else:
        ...
        if np.random.uniform(0,1) < 0.1:
            reg = regularizers[random.randint(0, len(regularizers) - 1)]
            if reg == 'l1l2':
                config["bias_regularizers"].append((reg, np.random.uniform(0,1) * 0.1, np.random.uniform(0,1) * 0.1))
            else:
